...
const allMenuItems = [
    ...
    { name: 'Konsultasi', route: 'superadmin.consultations.index', icon: 'M8 12h.01M12 12h.01M16 12h.01M21 12c0 4.418-4.03 8-9 8a9.863 9.863 0 01-4.255-.949L3 20l1.395-3.72C3.512 15.042 3 13.574 3 12c0-4.418 4.03-8 9-8s9 3.582 9 8z', roles: ['super_admin'] },
    { name: 'Register Tenant', route: 'superadmin.register.tenant', icon: 'M18 9v3m0 0v3m0-3h3m-3 0h-3m-2-5a4 4 0 11-8 0 4 4 0 018 0zM3 20a6 6 0 0112 0v1H3v-1z', roles: ['super_admin'] },
];
        ...
        <nav class="flex-1 py-6 px-2 space-y-1 overflow-y-auto overflow-x-hidden">
            <template v-for="item in menuItems" :key="item.name">
                <Link
                    ...
                    class="group relative flex items-center px-3 py-2.5 rounded-lg transition-all duration-200 whitespace-nowrap"
                    :class="route().current(item.route)
                        ? 'bg-[#1a1b20] text-blue-500'
                        : 'text-gray-500 hover:text-gray-200 hover:bg-[#1a1b20]'"
                    :title="props.isCollapsed ? item.name : ''"
                >
                    <!-- Icon wrapper dengan relative untuk dot badge -->
                    <div class="relative flex-shrink-0">
                        <svg class="w-6 h-6 transition-colors duration-200"
                             fill="none" viewBox="0 0 24 24" stroke="currentColor">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" :d="item.icon" />
                        </svg>
                        <!-- Red dot saat sidebar collapsed -->
                        <span
                            v-if="item.route === 'superadmin.consultations.index' && $page.props.unreadConsultationsCount > 0 && props.isCollapsed"
                            class="absolute -top-0.5 -right-0.5 w-2.5 h-2.5 rounded-full bg-red-500 ring-2 ring-[#0a0b0d]"
                        ></span>
                    </div>
                    ...
                    <!-- Red dot saat sidebar expanded -->
                    <span
                        v-if="item.route === 'superadmin.consultations.index' && $page.props.unreadConsultationsCount > 0 && !props.isCollapsed"
                        class="ml-auto w-2.5 h-2.5 rounded-full bg-red-500 flex-shrink-0"
                    ></span>
                </Link>
            </template>
        </nav>
